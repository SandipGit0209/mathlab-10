(() => {
  ...
  function polygon(svg, points, cls="shape-fill") {
    svg.appendChild(E("path", { d: P(points), class: cls }));
  }
  ...
  function drawCuboid(s,v) {
    const [w,h] = pair(v.l, v.h, 220, 150);
    const d = Math.min(85, Math.max(38, v.b * 5.5));
    const dx = d, dy = d * 0.55;
    const x = (600 - (w + dx)) / 2;
    const y = 170;
    const A = [x, y], B = [x+w, y], C = [x+w, y+h], D = [x, y+h];
    const A2 = [x+dx, y-dy], B2 = [x+w+dx, y-dy], C2 = [x+w+dx, y+h-dy], D2 = [x+dx, y+h-dy];
    polygon(s, [A2, B2, B, A], "shape-fill-2");
    polygon(s, [B2, C2, C, B], "shape-fill");
    polygon(s, [A, B, C, D], "shape-fill");
    s.appendChild(E("line", {x1:A[0], y1:A[1], x2:A2[0], y2:A2[1], class:"guide"}));
    s.appendChild(E("line", {x1:D[0], y1:D[1], x2:D2[0], y2:D2[1], class:"guide"}));
    T(s, (D[0]+C[0])/2, C[1]+34, `l = ${f(v.l)} ${state.unit}`);
    T(s, A[0]-14, (A[1]+D[1])/2, `h = ${f(v.h)} ${state.unit}`, "end");
    T(s, (A2[0]+B2[0])/2 + 14, (A2[1]+B2[1])/2 - 10, `b = ${f(v.b)} ${state.unit}`);
  }

  function drawCylinder(s,v) {
    const r = Math.min(95, Math.max(42, v.r * 11));
    const h = Math.min(190, Math.max(95, v.h * 9));
    const cx = 300, top = 125, ry = Math.max(15, r * 0.26);
    s.appendChild(E("ellipse", {cx, cy:top, rx:r, ry, class:"shape-fill-2"}));
    s.appendChild(E("line", {x1:cx-r, y1:top, x2:cx-r, y2:top+h, class:"shape-fill"}));
    s.appendChild(E("line", {x1:cx+r, y1:top, x2:cx+r, y2:top+h, class:"shape-fill"}));
    s.appendChild(E("ellipse", {cx, cy:top+h, rx:r, ry, class:"shape-fill"}));
    s.appendChild(E("line", {x1:cx, y1:top, x2:cx+r, y2:top, class:"guide"}));
    T(s, cx+r/2, top-14, `r = ${f(v.r)} ${state.unit}`);
    T(s, cx+r+18, top+h/2, `h = ${f(v.h)} ${state.unit}`, "start");
  }

  function drawCone(s,v) {
    const r = Math.min(100, Math.max(42, v.r * 11));
    const h = Math.min(220, Math.max(100, v.h * 9));
    const cx = 300, by = 340, ty = by - h, ry = Math.max(15, r * 0.24);
    ...
    s.appendChild(E("line", {x1:cx, y1:ty, x2:cx, y2:by, class:"guide"}));
    T(s, cx+r/2, by-14, `r = ${f(v.r)} ${state.unit}`);
    T(s, cx+18, (ty+by)/2, `h = ${f(v.h)} ${state.unit}`, "start");
  }
  ...
  function drawCylinderCone(s,v) {
    const r = Math.min(92, Math.max(42, v.r * 10));
    const hc = Math.min(115, Math.max(60, v.hc * 6.5));
    const hn = Math.min(110, Math.max(55, v.hn * 7.5));
    const cx = 300, join = 195, top = join - hn, bottom = join + hc, ry = Math.max(14, r * 0.22);
    s.appendChild(E("path", {d:`M ${cx} ${top} L ${cx-r} ${join} L ${cx+r} ${join} Z`, class:"shape-fill-2"}));
    s.appendChild(E("ellipse", {cx, cy:join, rx:r, ry, class:"shape-fill-2"}));
    s.appendChild(E("line", {x1:cx-r, y1:join, x2:cx-r, y2:bottom, class:"shape-fill"}));
    s.appendChild(E("line", {x1:cx+r, y1:join, x2:cx+r, y2:bottom, class:"shape-fill"}));
    s.appendChild(E("ellipse", {cx, cy:bottom, rx:r, ry, class:"shape-fill"}));
    T(s, cx+r+16, (join+bottom)/2, `h₁ = ${f(v.hc)} ${state.unit}`, "start");
    T(s, cx+18, (top+join)/2, `h₂ = ${f(v.hn)} ${state.unit}`, "start");
  }

  function drawPrismPyramid(s,v) {
    const a = Math.min(145, Math.max(72, v.a * 9));
    const hp = Math.min(120, Math.max(58, v.hp * 6.5));
    const hy = Math.min(100, Math.max(48, v.hy * 7));
    const dx = a * 0.42, dy = a * 0.24;
    const cx = 300, baseY = 360;
    const FTL=[cx-a/2, baseY-hp], FTR=[cx+a/2, baseY-hp], FBR=[cx+a/2, baseY], FBL=[cx-a/2, baseY];
    const BTL=[FTL[0]+dx, FTL[1]-dy], BTR=[FTR[0]+dx, FTR[1]-dy], BBR=[FBR[0]+dx, FBR[1]-dy], BBL=[FBL[0]+dx, FBL[1]-dy];
    polygon(s, [BTL, BTR, FTR, FTL], "shape-fill-2");
    polygon(s, [FTR, BTR, BBR, FBR], "shape-fill");
    polygon(s, [FTL, FTR, FBR, FBL], "shape-fill");
    const apex = [(BTL[0]+BTR[0])/2, (BTL[1]+BTR[1])/2 - hy];
    polygon(s, [apex, BTR, FTR], "shape-fill-2");
    polygon(s, [apex, FTR, FTL], "shape-fill-2");
    polygon(s, [apex, FTL, BTL], "shape-fill-2");
    T(s, (FBL[0]+FBR[0])/2, FBR[1]+32, `a = ${f(v.a)} ${state.unit}`);
    T(s, FTR[0]+24, (FTR[1]+FBR[1])/2, `h₁ = ${f(v.hp)} ${state.unit}`, "start");
    T(s, apex[0]+18, (apex[1]+(BTL[1]+BTR[1])/2)/2, `h₂ = ${f(v.hy)} ${state.unit}`, "start");
  }

  function drawCrystal(s,v) {
    const a = Math.min(130, Math.max(66, v.a * 8.5));
    const hp = Math.min(98, Math.max(46, v.hp * 6));
    const ht = Math.min(86, Math.max(40, v.ht * 6));
    const hb = Math.min(86, Math.max(40, v.hb * 6));
    const dx = a * 0.38, dy = a * 0.22;
    const cx = 300, baseY = 305;
    const FTL=[cx-a/2, baseY-hp], FTR=[cx+a/2, baseY-hp], FBR=[cx+a/2, baseY], FBL=[cx-a/2, baseY];
    const BTL=[FTL[0]+dx, FTL[1]-dy], BTR=[FTR[0]+dx, FTR[1]-dy], BBR=[FBR[0]+dx, FBR[1]-dy], BBL=[FBL[0]+dx, FBL[1]-dy];
    polygon(s, [BTL, BTR, FTR, FTL], "shape-fill-2");
    polygon(s, [FTR, BTR, BBR, FBR], "shape-fill");
    polygon(s, [FTL, FTR, FBR, FBL], "shape-fill");
    const topApex = [(BTL[0]+BTR[0])/2, (BTL[1]+BTR[1])/2 - ht];
    const bottomApex = [(FBL[0]+FBR[0])/2 + dx/2, (FBL[1]+FBR[1])/2 + hb];
    polygon(s, [topApex, BTR, FTR], "shape-fill-2");
    polygon(s, [topApex, FTR, FTL], "shape-fill-2");
    polygon(s, [topApex, FTL, BTL], "shape-fill-2");
    polygon(s, [FBL, FBR, bottomApex], "shape-fill-2");
    polygon(s, [FBR, BBR, bottomApex], "shape-fill-2");
    polygon(s, [FBL, BBL, bottomApex], "shape-fill-2");
    T(s, FTR[0]+24, (FTL[1]+FBL[1])/2, `h = ${f(v.hp)} ${state.unit}`, "start");
    T(s, topApex[0]+18, (topApex[1]+FTL[1])/2, `p₁ = ${f(v.ht)} ${state.unit}`, "start");
    T(s, bottomApex[0]+18, (bottomApex[1]+FBL[1])/2, `p₂ = ${f(v.hb)} ${state.unit}`, "start");
  }

  function drawDoublePyramid(s,v) {
    const a = Math.min(160, Math.max(82, v.a * 9));
    const h1 = Math.min(120, Math.max(55, v.h1 * 6.5));
    const h2 = Math.min(120, Math.max(55, v.h2 * 6.5));
    const dx = a * 0.45, dy = a * 0.26;
    const cx = 300, cy = 220;
    const L = [cx-a/2, cy], R = [cx+a/2, cy], Tm = [cx+dx, cy-dy], Bm = [cx-dx, cy+dy];
    const top = [cx, cy-h1], bottom = [cx, cy+h2];
    polygon(s, [top, Tm, R], "shape-fill-2");
    polygon(s, [top, R, Bm], "shape-fill");
    polygon(s, [top, Bm, L], "shape-fill-2");
    polygon(s, [top, L, Tm], "shape-fill");
    polygon(s, [bottom, Tm, R], "shape-fill");
    polygon(s, [bottom, R, Bm], "shape-fill-2");
    polygon(s, [bottom, Bm, L], "shape-fill");
    polygon(s, [bottom, L, Tm], "shape-fill-2");
    s.appendChild(E("path", {d: P([L,Tm,R,Bm]), class:"guide"}));
    T(s, top[0]+16, (top[1]+cy)/2, `h₁ = ${f(v.h1)} ${state.unit}`, "start");
    T(s, bottom[0]+16, (bottom[1]+cy)/2, `h₂ = ${f(v.h2)} ${state.unit}`, "start");
    T(s, cx, cy+20, `a = ${f(v.a)} ${state.unit}`);
  }
